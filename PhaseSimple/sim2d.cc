...
Csim2d::InitBound(void)
{
    ...
    // finally setting of average values of fl at cell-boundries
    for (i = 0; i < DimInfo->Nxy; i++) {
        FLmx[i] = .5 * (FL[i] + FL[i-1]);
    }
    /*for (j = DimInfo->Nx; j < DimInfo->Nxy; j++) {
		FLmy[j] = .5 * (FL[j] + FL[j-DimInfo->Nx]);
	}*/
    return 0;
}
...
Csim2d::InitRegion(istream &input)
{
    ...
    // Additional parameters for KWC model- 30.1.14 currently disabled
	/*
...
Csim2d::InitTemperature(void)
{
    ...
    theta_over = new double[DimInfo->Nxy];
    if (theta_over == NULL) {
...
Csim2d::StabilCond()
{
	...
	// stability condition for phi field
    //return TimeInfo->tWidth < tauPhi / (alfa * alfa) * XQuadrat * 0.25; 
	// stability condition for concentration
	//return TimeInfo->tWidth < XQuadrat / (2. * DC[1]) ;
	return true;
}
